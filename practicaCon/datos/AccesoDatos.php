...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton, se accede por getModelo
    ...
    public function gePedidos ($codigo):array {
        $tpro = [];
        // Sobre la conexión PDO prepara la consulta;
        $stmt_productos  = $this->dbh->prepare("select * from Pedido where cod_cliente = :cod_cliente");
        ...
        if ( $stmt_productos->execute() ){
            ...
            // Mientra $cod_cliente no sea false, sea un objeto
    ...
    public function getCliente (String $nombre, String $clave) {
        $cli = false;
        $stmt_cliente   = $this->dbh->prepare("select * from clientes where cod_cliente =?login");
        $stmt_cliente->setFetchMode(PDO::FETCH_CLASS, 'Usuario');
        $stmt_cliente->bindParam(':login', $nombre);
        if ( $stmt_cliente->execute($clave) ){
             // Solo hay un objeto
    ...
    // UPDATE
    public function modUsuario($cod_cliente):bool{
      
        $stmt_modcli   = $this->dbh->prepare("update clientes set veces=veces+1 where cod_cliente=:cod_cliente");
        $stmt_modcli->bindValue(':cod_cliente',$cod_cliente->login);
        $stmt_modcli->execute();
